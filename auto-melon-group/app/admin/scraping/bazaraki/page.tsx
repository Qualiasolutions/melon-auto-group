...
import { ArrowLeft, Search, Loader2, ShoppingBag, CheckCircle2, XCircle, Download } from "lucide-react"
...
export default function BazarakiScraper() {
  ...
  const handleScrape = async () => {
    if (!url.trim()) {
      setError("Please enter a Bazaraki URL")
      return
    }

    if (!url.includes('bazaraki.com')) {
      setError("Please enter a valid Bazaraki.com URL")
      return
    }
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(data.error || 'Failed to scrape Bazaraki')
      }
      ...
      setSuccess(`Successfully scraped vehicle from Bazaraki!`)

    } catch (err: any) {
      console.error('Error scraping:', err)
      setError(err.message || 'Failed to scrape Bazaraki. Please try again.')
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-8">
          ...
          <div className="flex items-start justify-between">
            <div>
              <div className="flex items-center gap-3 mb-2">
                ...
                <div>
                  <h1 className="text-3xl font-bold text-white">Bazaraki Scraper</h1>
                  <p className="text-slate-300 text-sm">Cyprus Marketplace</p>
                </div>
              </div>
              <p className="text-slate-400 mt-4 max-w-2xl">
                Import vehicle listings from Bazaraki.com with comprehensive details and images using Firecrawl technology.
              </p>
            </div>
      ...
      <Card className="border-2 border-slate-200 shadow-2xl">
        <CardHeader className="bg-gradient-to-r from-slate-50 to-white">
          <CardTitle className="text-2xl flex items-center gap-2">
            ...
            Import from Bazaraki
          </CardTitle>
          <CardDescription className="text-base">
            Paste a Bazaraki.com vehicle listing URL to extract vehicle details
          </CardDescription>
        </CardHeader>
          ...
          <div className="space-y-3">
            ...
            <div className="flex gap-3">
              <Input
                ...
                className="flex-1 h-14 text-base border-2 border-slate-200 focus:border-cyan-600 focus:ring-cyan-600 rounded-xl"
                disabled={loading}
              />
              <Button
                onClick={handleScrape}
                disabled={loading || !url.trim()}
                className="bg-gradient-to-r from-cyan-600 to-teal-600 hover:from-cyan-700 hover:to-teal-700 shadow-lg hover:shadow-xl min-w-[160px] h-14 text-base font-semibold rounded-xl"
              >
                {loading ? (
                  <>
          ...
          <CardContent className="pt-6">
            <div className="border-2 border-slate-200 rounded-xl p-6 space-y-6">
              ...
              <div className="bg-cyan-50 border-2 border-cyan-200 rounded-lg p-4">
                <p className="text-sm text-cyan-700 font-medium">Price</p>
              ...
              {vehicle.url && (
                <div>
                  ...
                  >
                    View Original Listing on Bazaraki →
                  </a>
                </div>
              )}
            </div>
      ...
      {/* Info Card */}
      <Card className="border-2 border-cyan-200 bg-gradient-to-br from-cyan-50 to-cyan-100/50 shadow-xl">
        <CardHeader>
        ...
        <CardContent className="space-y-3 text-sm text-cyan-800">
          <p>
            <strong>1. Find a Listing:</strong> Browse Bazaraki.com and find a vehicle listing you want to import.
          </p>
          <p>
            <strong>2. Copy URL:</strong> Copy the listing's URL from your browser.
          </p>
          <p>
            <strong>3. Paste & Scrape:</strong> Paste the URL above and click "Scrape Data" - our Firecrawl-powered scraper will extract all vehicle details.
          </p>
          <p>
          ...
          <p className="text-xs text-cyan-600 mt-4">
            <strong>Note:</strong> Scraping takes 10-20 seconds per listing. The data will be pre-filled in the vehicle form where you can review and adjust before saving.
          </p>
        </CardContent>
      </Card>
    </div>
