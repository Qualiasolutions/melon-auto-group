...
// Mapping of models to their image URLs
...
async function downloadAllImages() {
  console.log('📥 Downloading images from Bazaraki CDN...\n')

  const imageMapping: Record<string, string[]> = {}
  let totalDownloaded = 0
  let totalFailed = 0

  for (const [vehicleKey, urls] of Object.entries(vehicleImageMapping)) {
    console.log(`📂 Processing ${vehicleKey}...`)
    const localPaths: string[] = []

    for (let i = 0; i < urls.length; i++) {
      const url = urls[i]
      const filename = `${vehicleKey.replace(/\s+/g, '-').toLowerCase()}-${i + 1}.webp`
      const filepath = path.join(imagesDir, filename)
      const publicPath = `/images/vehicles/${filename}`
    ...
    imageMapping[vehicleKey] = localPaths
  }
